...
## Q14 (task T82, consumed, filed-at 2026-09-09T12:11:59Z) — May block-card work proceed without the external engine contract?
...
**A:** A — proceed without the external prerequisite, starting with legacy block visibility while retaining the full issue acceptance.

Decided by Chris directly in the Claude Code operator session on 2026-09-09, when presented with this question alongside the T82 plan gate; he selected "Approve + proceed legacy-first" over the option that would have left T82 blocked until MrZoller/opencode-factory#147 lands. This is his own answer, not an operator answer under the agreed-recommendation delegation — Q14 carries an explicit `No recommendation` rationale and so falls outside that delegation. Earlier the same session he had already declined a "wait for #147 first" option when choosing to import #115, which is consistent with this answer but was not treated as sufficient to answer on his behalf.

Grounds as he was shown them: issue #115 states the card can land against legacy free-text first and pick up structured fields when the grammar ships, so legacy-first delivery is authorized by the issue itself; opencode-factory#147 was directly verified OPEN on 2026-09-09. Remove MrZoller/opencode-factory#147 from T82's deps and permit legacy-first work. Retain all structured-card and release-action acceptance; do not invent an engine contract; stop for reclassification or decomposition rather than inventing a release protocol if the missing contract prevents safe completion. #115 remains open until full acceptance is met.

## Q15 (task T82, answered, filed-at 2026-09-12T12:15:14Z) — Must the three refuted peer-input findings on PR #116 be fixed?
Context: Observable failure: a maintainer opens the dashboard and a single peer machine returning malformed or oversized task data blanks that peer out, or the page throws while drawing a block card. Three review findings described exactly this on the new block-card surface and were closed as invalid without a code change, so the behaviour is still present on the shipped head.
Engine detail: T82's parked branch is `factory/t82-block-cards`, head `3f3ec50`, open as PR #116. Codex raised two P1 findings (unbounded peer task count, uncapped peer dependency arrays) and one P2 (unvalidated peer task identity interpolation). The independent verifier classified all three invalid and posted gate-complete refutation replies; the threads remain open and unresolved. The refutation is contradicted by the code in that same head: `readerData` returns `result.data` unbounded and unvalidated, `isReaderResult` checks only `isRecord(value.data)`, `isRepository` inspects no task, `renderTaskBlocks` iterates every task with no cap while `MAX_PLAN_TASKS = 256` goes unapplied on that path, `${task.id ?? "?"}` interpolates a non-null object and throws `TypeError: Cannot convert object to primitive value` because `??` does not catch it, and `taskDependencyStrings` filters by type without applying `MAX_TASK_DEPENDENCIES = 32`. The P2 is the same crash mechanism the shepherd itself classified blocking and fixed one round earlier in this PR, so the two classifications are mutually inconsistent.
Options considered: A — fix all three findings at the peer boundary and keep the PR (recommended) / B — accept the refutation and merge the head unchanged
Option A: Bound and validate peer task data once where it enters the block-card path, rather than patching each reporting site. Cap the task list at `MAX_PLAN_TASKS`, cap dependency arrays at `MAX_TASK_DEPENDENCIES`, and coerce or reject non-string task `id` and `title` before interpolation. Add regressions for oversized task lists, oversized dependency arrays, and object-valued identity fields. Then reply on the three open threads withdrawing the refutation and resolve them.
Owner: factory-ui implementation owner.
Day-to-day consequence: a malformed or hostile peer can no longer blank out a peer card or throw while rendering; the dashboard degrades honestly instead.
Cost or risk: one more fix-and-review round on PR #116; the boundary fix is slightly larger than three site patches but prevents the next instance of the same class.
Option B: Merge `3f3ec50` unchanged on the strength of the verifier's refutation.
Owner: Chris.
Day-to-day consequence: the peer path ships unbounded and unvalidated; a single bad peer response can blank a peer or throw during render.
Cost or risk: ships a defect class that T82's own acceptance required to be covered, and leaves two mutually inconsistent classifications standing as precedent.
Recommendation rationale: The refutation is contradicted by a complete four-function trace in the shipped head, and the P2 duplicates a finding the same shepherd already accepted as blocking.
**A:** A — fix all three findings at the peer boundary.

Decided by Chris on 2026-09-12 in the Claude Code operator session. Shown the trace above and the inconsistency between the two classifications, he answered "Fix them". This is an owner override of the verifier's invalid classification, routed through the answered-question channel because the shepherd re-classifies human findings rather than treating them as authoritative (opencode-factory#113). Implement Option A as part of T82's acceptance, which already required bounded parsing and hostile-input coverage on the peer path; the existing PR #116 is adopted at ship time rather than reopened.
